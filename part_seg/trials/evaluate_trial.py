...
def evaluate():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            pointclouds_pl = tf.get_default_graph().get_tensor_by_name("Placeholder:0")
            cls_labels_pl = tf.get_default_graph().get_tensor_by_name("Placeholder_2:0")
            is_training_pl = tf.get_default_graph().get_tensor_by_name("Placeholder_3:0")
            labels_pl = tf.get_default_graph().get_tensor_by_name("Placeholder_1:0")
            pred = tf.get_default_graph().get_tensor_by_name("fc2/BiasAdd:0")
            end_points = pred
        ...
        #nodes = [v.name for v in tf.get_default_graph().as_graph_def().node]
