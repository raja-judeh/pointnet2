...
import provider
import tf_util
import part_dataset_multipoints
...
parser.add_argument('--num_point_pairs', type=int, default=100, help='Point-Pairs Number Per Part[default: 100]')
...
NUM_POINT_PAIRS_PER_PART = FLAGS.num_point_pairs
NUM_POINT_PAIRS = NUM_POINT_PAIRS_PER_PART*15 + NUM_POINT_PAIRS_PER_PART*6
...
TRAIN_DATASET = part_dataset_multipoints.PartNormalDataset(root=DATA_PATH, npoints=NUM_POINT, npoint_pairs=NUM_POINT_PAIRS_PER_PART, split='trainval')
TEST_DATASET = part_dataset_multipoints.PartNormalDataset(root=DATA_PATH, npoints=NUM_POINT, npoint_pairs=NUM_POINT_PAIRS_PER_PART, split='test')

ROTATIONS_PATH = os.path.join(DATA_DIR, 'rotations.txt')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            pointclouds_pl, labels_pl, cls_labels_pl, pp_idx_pl, pp_labels_pl = MODEL.placeholder_inputs(BATCH_SIZE, NUM_POINT, NUM_POINT_PAIRS)
            ...
            # Get model and loss 
            pred, pp_pred, end_points = MODEL.get_model(pointclouds_pl, cls_labels_pl, pp_idx_pl, is_training_pl, bn_decay=bn_decay)
            loss, pp_loss = MODEL.get_loss(pred, labels_pl, pp_pred, pp_labels_pl, end_points)
            tf.summary.scalar('loss', loss)
            tf.summary.scalar('pp_loss', pp_loss)
...
def get_batch(dataset, idxs, start_idx, end_idx):
    ...
    batch_pp_idx = np.zeros((bsize,NUM_POINT_PAIRS,2), dtype=np.int32)
    batch_pp_label = np.zeros((bsize,NUM_POINT_PAIRS), dtype=np.float32)
    for i in range(bsize):
        ps,normal,seg,cls,pp_idx,pp_label = dataset[idxs[i+start_idx]]
        batch_data[i,:,0:3] = ps
        batch_data[i,:,3:6] = normal
        batch_label[i,:] = seg
        batch_cls_label[i] = cls
        batch_pp_idx[i] = pp_idx
        batch_pp_label[i] = pp_label
    return batch_data, batch_label, batch_cls_label, batch_pp_idx, batch_pp_label
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_data, batch_label, batch_cls_label, batch_pp_idx, batch_pp_label = get_batch(TRAIN_DATASET, train_idxs, start_idx, end_idx)
        ...
        summary, step, _, loss_val, pp_loss_val, pred_val, _ = sess.run([ops['merged'], ops['step'],
            ops['train_op'], ops['loss'], ops['pp_loss'], ops['pred'], ops['pp_pred']], feed_dict=feed_dict)
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    batch_pp_idx = np.zeros((BATCH_SIZE, NUM_POINT_PAIRS, 2)).astype(np.int32)
    batch_pp_label = np.zeros((BATCH_SIZE, NUM_POINT_PAIRS)).astype(np.float32)
    for batch_idx in range(num_batches):
        if batch_idx %20==0:
        ...
        cur_batch_data, cur_batch_label, cur_batch_cls_label, cur_batch_pp_idx, cur_batch_pp_label = get_batch(TEST_DATASET, test_idxs, start_idx, end_idx)
        if cur_batch_size == BATCH_SIZE:
            batch_data = cur_batch_data
            batch_label = cur_batch_label
            batch_cls_label = cur_batch_cls_label
            batch_pp_idx = cur_batch_pp_idx
            batch_pp_label = cur_batch_pp_label
        else:
            ...
            batch_pp_idx[0:cur_batch_size] = cur_batch_pp_idx
            batch_pp_label[0:cur_batch_size] = cur_batch_pp_label
        ...
        feed_dict = {ops['pointclouds_pl']: batch_data,
                     ...
                     ops['pp_labels_pl']: batch_pp_label,
                     ops['is_training_pl']: is_training}
        summary, step, loss_val, pp_loss_val, pred_val, _ = sess.run([ops['merged'], ops['step'],
            ops['loss'], ops['pp_loss'], ops['pred'], ops['pp_pred']], feed_dict=feed_dict)
    ...
    log_string('eval mean total_loss: %f' % (loss_sum / float(len(TEST_DATASET)/BATCH_SIZE)))
    log_string('eval mean pp_loss: %f' % (pp_loss_sum / float(len(TEST_DATASET)/BATCH_SIZE)))
