...
ROTATIONS_PATH = os.path.join(DATA_DIR, 'rotations', 'selected_deg30.txt')
ROT_MATS = provider.get_rotations(ROTATIONS_PATH)
ROT_MATS = np.concatenate((ROT_MATS, np.expand_dims(np.eye(3), axis=0)), axis=0)
            ...
            for i in range(NUM_GPUS):
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    with tf.device('/gpu:%d'%(i)), tf.name_scope('gpu_%d'%(i)) as scope:
                        ...
                        cls_labels_batch = tf.slice(cls_labels_pl,
                            [i*DEVICE_BATCH_SIZE], [DEVICE_BATCH_SIZE])

                        pred, end_points = MODEL.get_model(pc_batch, cls_labels_batch,
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        log_string('** Batch %03d / %03d **' % (batch_idx+1, num_batches))
        start_idx = batch_idx * BATCH_SIZE
        end_idx = (batch_idx+1) * BATCH_SIZE
        batch_data, batch_label, batch_cls_label = get_batch(TRAIN_DATASET, train_idxs, start_idx, end_idx)

        # Augment batched point clouds by rotation and jittering
        for R_idx, R in enumerate(ROT_MATS):
            num_rotations = ROT_MATS.shape[0]
            rotated_batch_data = provider.rotate_batch_data_R(batch_data, R)
            ...
            if (R_idx+1)%50 == 0:
                log_string(' -- R %03d / %03d rotations --' % (R_idx+1, num_rotations))
                

        #log_string(' -- %03d / %03d --' % (batch_idx+1, num_batches))
        log_string('mean loss: %f' % (loss_sum / len(ROT_MATS)))
