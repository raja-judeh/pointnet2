...
def pointnet_sa_module_rand_tree_triples(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope, nshuffles=1, bn=True, knn=False, use_xyz=True, use_nchw=False):
    ''' PointNet Set Abstraction (SA) Module
        Input:
            xyz: (batch_size, ndataset, 3) TF tensor
            points: (batch_size, ndataset, channel) TF tensor
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i, num_out_channel in enumerate(mlp):
            ...
            point_idxs = np.arange(new_points_tr.get_shape()[0].value)
            point_idxs = np.resize(point_idxs, (new_points_tr.get_shape()[0].value * nshuffles,))
            ...
            # Point Feature Embedding
            if use_nchw: new_points = tf.transpose(new_points, [0,3,1,2])
            new_points = tf_util.conv2d(new_points, num_out_channel, [1,3],
...
def pointnet_sa_module_msg_rand_triples(xyz, points, npoint, radius_list, nsample_list, mlp_list, is_training, bn_decay, scope, pooling='max', nshuffles=1, bn=True, use_xyz=True, use_nchw=False):
    ''' PointNet Set Abstraction (SA) module with Multi-Scale Grouping (MSG)
        Input:
            xyz: (batch_size, ndataset, 3) TF tensor
            points: (batch_size, ndataset, channel) TF tensor
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i in range(len(radius_list)):
            ...
            if use_nchw: grouped_points = tf.transpose(grouped_points, [0,3,1,2])
            print('grouped_points', grouped_points.get_shape())

            # Process point triples in the first layer
            grouped_points = tf_util.conv2d(grouped_points, mlp_list[i][0], [1,3],
                                            padding='VALID', stride=[1,1], bn=bn, is_training=is_training,
                                            scope='conv%d_%d'%(i,0), bn_decay=bn_decay)

            for j,num_out_channel in enumerate(mlp_list[i]):
                if j==0: continue #start from second layer
                grouped_points = tf_util.conv2d(grouped_points, num_out_channel, [1,1],
                                                padding='VALID', stride=[1,1], bn=bn, is_training=is_training,
                                                scope='conv%d_%d'%(i,j), bn_decay=bn_decay)
